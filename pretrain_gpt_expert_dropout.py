...
def _add_moe_signals_args(p):
    """Add MoE signals logging + expert-dropout arguments.

    Expert dropout (Tier-2 expert-regularization baseline): during TRAINING only,
    each MoE router randomly masks a fraction of experts before top-k selection, so
    tokens are forced onto surviving experts. This regularizes the model toward
    tolerating missing/unavailable experts -- a direct stressor of the same
    routing-robustness property DRMoET targets, via a generic dropout mechanism
    rather than loss reweighting. At eval time no experts are dropped.
    """
    ...
    g.add_argument('--expert-dropout-prob', type=float, default=0.0,
                   help='Per-expert drop probability during training (0 disables). '
                        'The dropped set is re-sampled once per optimizer step and per MoE '
                        'layer, is identical on every rank, and always leaves at least top-k '
                        'experts so routing stays satisfiable. No experts are dropped at eval.')
    return p


# Expert keep-masks for the current optimizer step, keyed by layer. Cleared whenever
# the iteration advances so the dict never holds more than one step of masks.
_ED_MASK_CACHE = {}
_ED_MASK_ITER = None


def _expert_keep_mask(num_experts, topk, layer_number, device):
    """Deterministic per-(step, layer) expert keep-mask.

    The mask is seeded from (args.seed, curr_iteration, layer_number) and sampled on
    the CPU, which buys three properties that a plain torch.rand(..., device=cuda)
    does not:

    1. Rank-consistent: every DP/EP rank derives the same mask, so the model sees one
       coherent subset of experts per step instead of a different subset per shard.
    2. Recomputation-safe: the mask is a pure function of (seed, iteration, layer), so
       a forward that is re-run under activation recomputation rebuilds the identical
       mask. Sampling from the CUDA RNG would redraw and silently desynchronise the
       recomputed forward from the one the gradients were built for.
    3. Side-effect free: it neither advances the CUDA RNG (which ordinary dropout and
       init depend on) nor forces a host-device sync to count survivors.

    At least topk experts always survive, so top-k selection stays well defined.
    """
    global _ED_MASK_ITER
    args = get_args()
    p_drop = float(getattr(args, 'expert_dropout_prob', 0.0) or 0.0)
    iteration = int(getattr(args, 'curr_iteration', 0) or 0)
    layer = -1 if layer_number is None else int(layer_number)

    if _ED_MASK_ITER != iteration:
        _ED_MASK_CACHE.clear()
        _ED_MASK_ITER = iteration

    key = (layer, num_experts, topk, str(device))
    if key in _ED_MASK_CACHE:
        return _ED_MASK_CACHE[key]

    generator = torch.Generator()  # CPU generator: see (2) and (3) above
    generator.manual_seed(
        (int(getattr(args, 'seed', 1234)) * 1000003 + iteration * 9176 + (layer + 1) * 97)
        & 0x7FFFFFFF
    )
    draw = torch.rand(num_experts, generator=generator)
    keep = draw >= p_drop
    if int(keep.sum()) < topk:
        # Too many experts drawn out: keep the topk highest draws so the survivor set
        # is still random but large enough for top-k routing.
        keep = torch.zeros(num_experts, dtype=torch.bool)
        keep[torch.topk(draw, topk).indices] = True

    mask = keep.to(device=device)
    _ED_MASK_CACHE[key] = mask
    return mask


def _install_router_hooks(model):
    """Install a single forward hook per MoE router.

    Both MoE-signal logging and expert dropout need to reach inside the router,
    between gating and routing, so they are served by one hook rather than two
    competing replacements of ``router.forward`` (a second replacement would shadow
    the first and silently stop ``last_gate_logits`` from being recorded).

    Expert dropout is the Tier-2 expert-regularization baseline: masking experts out
    of top-k selection during training forces tokens onto surviving experts, which
    stresses the same routing-robustness property DRMoET targets but through a generic
    dropout mechanism instead of loss reweighting.
    """
    args = get_args()
    log_signals = bool(getattr(args, 'log_moe_signals', False))
    p_drop = float(getattr(args, 'expert_dropout_prob', 0.0) or 0.0)

    if not log_signals and p_drop <= 0.0:
        return model
    ...
    for name, module in model.named_modules():
        ...
        def create_hook(router):
            def hooked_forward(input_tensor):
                # Mirror TopKRouter.forward: jitter -> gating -> routing.
                input_tensor = router.apply_input_jitter(input_tensor)
                logits = router.gating(input_tensor)

                if log_signals:
                    # Recorded pre-dropout so the logged router diagnostics measure the
                    # router's own preferences and stay comparable with baseline runs.
                    router.last_gate_logits = logits.detach()

                if p_drop > 0.0 and router.training:
                    keep = _expert_keep_mask(
                        logits.shape[-1], router.topk, router.layer_number, logits.device
                    )
                    logits = logits.masked_fill(~keep, float('-inf'))

                return router.routing(logits)

            return hooked_forward

        module.router.forward = create_hook(module.router)
        num_hooked += 1

    if p_drop > 0.0:
        print_rank_0(f">>> expert dropout p={p_drop} installed on {num_hooked} routers")

    if log_signals:
        def signals_forward(*fwd_args, **fwd_kwargs):
            """Enhanced forward pass that captures gate logits for logging."""
            model.gate_logits_buffer.clear()

            output = model._original_forward(*fwd_args, **fwd_kwargs)

            for name, module in model.named_modules():
                if hasattr(module, 'router'):
                    router = module.router
                    if getattr(router, 'last_gate_logits', None) is not None:
...
def model_provider(pre_process=True, post_process=True) -> Union[GPTModel, megatron.legacy.model.GPTModel]:
    ...
    # Install the router hooks for MoE signal logging and/or expert dropout.
    if args.num_experts > 0:
        model = _install_router_hooks(model)
