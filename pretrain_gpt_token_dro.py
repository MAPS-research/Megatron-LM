...
def _add_moe_signals_args(p):
    """Add MoE signals logging + token-level DRO arguments.

    Token-level DRO/CVaR is the *control* for expert-level DRMoET: it applies the
    same distributionally-robust reweighting to the per-token loss distribution
    but WITHOUT any expert/routing attribution. If DRMoET beats this, the gain is
    expert/routing-specific rather than generic hard-example upweighting.
    Stateless per micro-batch (tokens do not persist across steps, so there is no
    EMA / dual variable to carry, unlike expert-DRO's mu).
    """
    ...
    t = p.add_argument_group('Token-level DRO (control baseline)')
    t.add_argument('--use-token-dro', action='store_true',
                   help='Enable token-level DRO/CVaR loss reweighting (control for expert DRMoET).')
    t.add_argument('--token-dro-mode', type=str, default='cvar',
                   choices=['cvar', 'entropic', 'focal'],
                   help="'cvar': mean of the worst-alpha token losses (Rockafellar-Uryasev). "
                        "'entropic': KL-ball / softmax(eta*loss) reweighting -- the exact token "
                        "analogue of expert DRMoET's mu update. "
                        "'focal': (1-p)^gamma hard-example weighting (Tier-2 generic-reweighting baseline).")
    t.add_argument('--token-dro-alpha', type=float, default=0.5,
                   help='CVaR tail fraction in (0,1]. alpha=1 recovers ERM; smaller = more robust.')
    t.add_argument('--token-dro-eta', type=float, default=1.0,
                   help='Entropic-DRO temperature. eta->0 recovers ERM; larger = more robust.')
    t.add_argument('--token-dro-gamma', type=float, default=2.0,
                   help='Focal-loss focusing parameter. gamma=0 recovers ERM; larger = more focus on hard tokens.')
    return p


def _token_dro_weights(token_loss_flat, mask_flat):
    """Per-token DRO weights (DETACHED) that preserve total mass = #valid tokens.

    Returns a_j >= 0 with sum_j a_j*mask_j == sum_j mask_j, so that
    loss[0] = sum_j a_j*mask_j*loss_j stays on the same (summed) scale as the
    standard ERM loss[0] = sum_j mask_j*loss_j -- keeping the LR / grad scale and
    the reported per-token loss interpretable. At the ERM limit (alpha=1 or
    eta->0) a_j == 1 exactly, so the objective degrades gracefully to ERM.
    Detaching the weights implements the stop-gradient on the DRO adversary: the
    model is optimized *given* the worst-case weights, matching sg(mu) in DRMoET.
    """
    args = get_args()
    with torch.no_grad():
        m = mask_flat.bool()
        n_valid = m.sum()
        if n_valid == 0:
            return torch.ones_like(token_loss_flat)
        a = torch.ones_like(token_loss_flat)
        lv = token_loss_flat[m]  # valid token losses only
        if args.token_dro_mode == 'cvar':
            alpha = float(args.token_dro_alpha)
            if alpha >= 1.0:
                return a  # ERM
            # threshold = (1-alpha) quantile; keep the worst-alpha tail, weight 1/alpha
            tau = torch.quantile(lv.float(), 1.0 - alpha)
            w = (token_loss_flat >= tau).to(token_loss_flat.dtype) / alpha
            # renormalize so masked weights sum exactly to n_valid (quantile ties -> mass drift)
            denom = (w * mask_flat).sum().clamp_min(1e-8)
            a = w * (n_valid / denom)
        elif args.token_dro_mode == 'entropic':  # a_j = n_valid * softmax(eta*loss)
            eta = float(args.token_dro_eta)
            if eta <= 0.0:
                return a  # ERM
            z = eta * (token_loss_flat - lv.max())  # shift for numerical stability
            e = torch.exp(z) * mask_flat
            a = e * (n_valid / e.sum().clamp_min(1e-8))
        else:  # focal: w_j = (1 - p_j)^gamma, p_j = exp(-loss_j); mass-preserving normalization
            gamma = float(args.token_dro_gamma)
            if gamma <= 0.0:
                return a  # ERM
            p = torch.exp(-token_loss_flat).clamp(0.0, 1.0)
            w = torch.pow((1.0 - p), gamma) * mask_flat
            a = w * (n_valid / (w.sum().clamp_min(1e-8)))
        return a
...
def loss_func(loss_mask: torch.Tensor, output_tensor: torch.Tensor, gate_logits=None):
    ...
    # Standard (ERM) summed loss -- always computed, used as 'lm loss base' and as
    # the training loss when token-DRO is disabled.
    base_sum = torch.sum(losses_flat * loss_mask)

    if getattr(args, 'use_token_dro', False):
        a = _token_dro_weights(losses_flat, loss_mask)          # detached, sum(a*mask)==n_valid
        robust_sum = torch.sum(a * losses_flat * loss_mask)     # same scale as base_sum
        loss = torch.cat([robust_sum.view(1), total_tokens.view(1)])
    else:
        loss = torch.cat([base_sum.view(1), total_tokens.view(1)])
    ...
    if getattr(args, 'use_token_dro', False):
        # Report the ERM/base loss alongside the robust loss so the robust-vs-base
        # gap (and its shrinkage over training) is visible, exactly as DRMoET logs
        # 'lm loss base'.
        reporting_base = torch.stack([base_sum.detach(), total_tokens]).contiguous()
        torch.distributed.all_reduce(reporting_base, group=mpu.get_data_parallel_group())
        metrics['lm loss base'] = (reporting_base[0], reporting_base[1])
