...
"""Pretrain GPT with contribution-magnitude weighted DRO support.

This variant weights per-expert contributions in the DRO loss by the
...
def _add_dro_args(p):
    ...
    g = p.add_argument_group('DRO μ update (activation-weighted)')
    ...
    # New argument for activation weighting
...
def model_provider(pre_process=True, post_process=True) -> Union[GPTModel, megatron.legacy.model.GPTModel]:
    ...
    print_rank_0('building GPT model with activation-weighted DRO support ...')
...
def forward_step(data_iterator, model: GPTModel):
    ...
    if args.num_experts > 0:
        ...
        return output_tensor, partial(loss_func, loss_mask, gate_logits=gate_logits, activation_norms=activation_norms)
    else:
